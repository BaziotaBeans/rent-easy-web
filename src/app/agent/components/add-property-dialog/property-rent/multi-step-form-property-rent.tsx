...
import { stepAddPropertySchemas } from "@/validations/stepAddPropertySchemas";
...
// Inferindo o tipo de união dos esquemas
type StepSchema = z.infer<(typeof stepAddPropertySchemas)[number]>;

// Resolver do esquema do passo atual
const resolver = (step: number) => zodResolver(stepAddPropertySchemas[step]);

export function MultiStepFormPropertyRent() {
  const [step, setStep] = useState(0);
  const totalSteps = stepAddPropertySchemas.length;
      ...
      try {
        ...
        if (uploadedURLS.length) {
          const formattedData: PropertyRequest = {
            ...
            paymentModality: updatedData.paymentModality,
            status: true,
            ...
            condominiumFee: updatedData.condominiumFee ? parseFloat(updatedData.condominiumFee) : 0,
